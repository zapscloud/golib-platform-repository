...
func (p *PaymentsMongoDBDao) List(filter string, sort string, skip int64, limit int64) (utils.Map, error) {
	...
	log.Println("Begin - Find All Collection Dao", platform_common.DbPlatformPayments)

	collection, ctx, err := mongo_utils.GetMongoDbCollection(p.client, platform_common.DbPlatformPayments)
	if err != nil {
		return nil, err
	}
...
func (p *PaymentsMongoDBDao) Get(payment_id string) (utils.Map, error) {
	...
	log.Println("accountMongoDao::Get:: Begin ", payment_id)
	...
	stages := []bson.M{}
	filter := bson.D{
		{Key: platform_common.FLD_PAYMENT_ID, Value: payment_id},
	...
	if !singleResult.Next(ctx) {
		...
		err := &utils.AppError{ErrorCode: "S30102", ErrorMsg: "Record Not Found", ErrorDetail: "Given Payments is not found"}
...
func (p *PaymentsMongoDBDao) Create(indata utils.Map) (utils.Map, error) {
	...
	log.Println("Save - End", indata[platform_common.FLD_PAYMENT_ID])
...
func (p *PaymentsMongoDBDao) Delete(payment_id string) (int64, error) {
	...
	log.Println("accountMongoDao::Delete - Begin ", payment_id)
